...
object LaunchSbtJar:
  def launch(jdkHome: Path, jarLocation: Path, arguments: Seq[String])(using
      Env,
      Logger
  ) =
    Zone:
      ...
      if error == -1 then
        getLogger.error(s"execve failed with ${errno.errno}")
  end launch

  def encode(args: Seq[String])(using Zone) =
